"! @testing BDEF:ZRAP100_R_TravelTP_ZVZ
CLASS zrap100_tc_travel_eml_zvz DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS zrap100_tc_travel_eml_zvz IMPLEMENTATION.
  METHOD class_setup.
    " create the test doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZRAP100_R_TravelTP_zvz' ) ) ).
  ...
  METHOD create_with_action.
    ...
    MODIFY ENTITIES OF ZRAP100_R_TravelTP_zvz
     ENTITY Travel
     CREATE FIELDS ( AgencyID TravelID CustomerID BeginDate EndDate Description TotalPrice BookingFee CurrencyCode )
    ...
    SELECT * FROM ZRAP100_R_TravelTP_zvz INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
